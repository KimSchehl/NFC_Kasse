...
if not exist "config.env" (
    ...
    powershell -NoProfile -Command "$b=[byte[]]::new(32);[System.Security.Cryptography.RandomNumberGenerator]::Create().GetBytes($b);$k=[System.Convert]::ToBase64String($b).TrimEnd('=').Replace('+','-').Replace('/','_');$id=[System.Guid]::NewGuid().ToString(); Set-Content config.env -Encoding UTF8 -Value @('# NFC-Kasse Configuration','# Edit this file before starting the backend.','# Changes take effect on the next server start.','','# Network interface to bind to (0.0.0.0 = all interfaces).','HOST=0.0.0.0','','# Port the backend listens on.','PORT=8000','','# Secret used to sign login tokens (JWT).','# IMPORTANT: Change this before using with real data!',\"SECRET_KEY=$k\",'','# Unique installation ID -- generated once, do not change.','# Needed to request feature license keys (PAGER, LEADERBOARD, ...).',\"INSTALLATION_ID=$id\",'','# Chip deposit in EUR (e.g. 3.00 for 3 Euro).','# Deducted automatically on first chip issuance; refunded on payout.','# Set to 0 to disable deposit logic.','CHIP_DEPOSIT=3.00','','# Leaderboard-Feature (kostenpflichtiges Add-on, braucht LEADERBOARD_LICENSE_KEY).','# true = aktiviert; false = deaktiviert (Standard).','LEADERBOARD=false','','# Pager-Feature (kostenpflichtiges Add-on, braucht PAGER_LICENSE_KEY).','# true = aktiviert; false = deaktiviert (Standard).','PAGER=false','','# URL-Pfad der Flutter Web App (nur aktiv wenn backend\webapp\ existiert).','# Nach Pfad-Aenderung neu bauen: flutter build web --release --base-href /neuer-pfad/','WEBAPP_ROUTE=/webapp')"
    if errorlevel 1 (
